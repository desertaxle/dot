...
class TaskRepositoryContract:
    ...
    def test_add_task(self):
        ...
        retrieved = self.repository.get(task_id)
    ...
    def test_list_tasks(self):
        ...
        task1_id = uuid4()
        task2_id = uuid4()
        task1 = Task(id=task1_id, title="Task 1", created_at=now, updated_at=now)
        task2 = Task(id=task2_id, title="Task 2", created_at=now, updated_at=now)
    ...
    def test_update_task(self):
        ...
        now = datetime.now(timezone.utc)
        task_id = uuid4()
        task = Task(id=task_id, title="Original", created_at=now, updated_at=now)
    ...
    def test_delete_task(self):
        ...
        now = datetime.now(timezone.utc)
        task_id = uuid4()
        task = Task(id=task_id, title="To delete", created_at=now, updated_at=now)

        self.repository.add(task)
        assert self.repository.get(task_id) is not None
...
class NoteRepositoryContract:
    ...
    def test_add_note(self):
        ...
        retrieved = self.repository.get(note_id)
    ...
    def test_list_notes(self):
        ...
        note1_id = uuid4()
        note2_id = uuid4()
        note1 = Note(id=note1_id, title="Note 1", created_at=now, updated_at=now)
        note2 = Note(id=note2_id, title="Note 2", created_at=now, updated_at=now)
        ...
        assert len(notes) == 2
        assert any(n.id == note1_id for n in notes)
        assert any(n.id == note2_id for n in notes)

    def test_update_note(self):
        """Repository can update a note."""
        now = datetime.now(timezone.utc)
        note_id = uuid4()
        note = Note(id=note_id, title="Original", created_at=now, updated_at=now)
    ...
    def test_delete_note(self):
        ...
        now = datetime.now(timezone.utc)
        note_id = uuid4()
        note = Note(id=note_id, title="To delete", created_at=now, updated_at=now)

        self.repository.add(note)
        assert self.repository.get(note_id) is not None
...
class EventRepositoryContract:
    ...
    def test_add_event(self):
        ...
        retrieved = self.repository.get(event_id)
    ...
    def test_list_events(self):
        ...
        assert len(events) == 2
        assert any(e.id == event1_id for e in events)
        assert any(e.id == event2_id for e in events)
    ...
    def test_delete_event(self):
        ...
        self.repository.add(event)
        assert self.repository.get(event_id) is not None
...
class ProjectRepositoryContract:
    ...
    def test_add_project(self):
        ...
        retrieved = self.repository.get(project_id)
    ...
    def test_list_projects(self):
        ...
        project2_id = uuid4()
        project1 = Project(
            id=project1_id, name="Project 1", created_at=now, updated_at=now
        )
        project2 = Project(
            id=project2_id, name="Project 2", created_at=now, updated_at=now
        )
        ...
        assert any(p.id == project1_id for p in projects)
        assert any(p.id == project2_id for p in projects)
    ...
    def test_update_project(self):
        ...
        project_id = uuid4()
        project = Project(
            id=project_id, name="Original", created_at=now, updated_at=now
        )
    ...
    def test_delete_project(self):
        ...
        project_id = uuid4()
        project = Project(
            id=project_id, name="To delete", created_at=now, updated_at=now
        )

        self.repository.add(project)
        assert self.repository.get(project_id) is not None
...
class LogEntryRepositoryContract:
    ...
    def test_add_log_entry(self):
        ...
        retrieved = self.repository.get(entry_id)
    ...
    def test_list_log_entries(self):
        ...
        note_id = uuid4()
        entry1 = LogEntry(
            id=entry1_id, log_id=log_id, task_id=task_id, entry_date=today
        )
        entry2 = LogEntry(
            id=entry2_id, log_id=log_id, note_id=note_id, entry_date=today
        )
        ...
        assert any(e.id == entry1_id for e in entries)
        assert any(e.id == entry2_id for e in entries)
    ...
    def test_delete_log_entry(self):
        ...
        log_id = uuid4()
        task_id = uuid4()
        entry = LogEntry(id=entry_id, log_id=log_id, task_id=task_id, entry_date=today)

        self.repository.add(entry)
        assert self.repository.get(entry_id) is not None

        self.repository.delete(entry_id)
        assert self.repository.get(entry_id) is None
    ...
    def test_get_by_log_id(self):
        ...
        event_id = uuid4()
        entry1 = LogEntry(
            id=entry1_id, log_id=log1_id, task_id=task_id, entry_date=today
        )
        entry2 = LogEntry(
            id=entry2_id, log_id=log1_id, note_id=note_id, entry_date=today
        )
        entry3 = LogEntry(
            id=entry3_id, log_id=log2_id, event_id=event_id, entry_date=today
        )
        ...
        log1_entries = self.repository.get_by_log_id(log1_id)
        assert len(log1_entries) == 2
        assert all(e.log_id == log1_id for e in log1_entries)
