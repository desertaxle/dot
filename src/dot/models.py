...
class Project(Base):
    ...
    # Relationships
    log_entries: Mapped[list["LogEntry"]] = relationship("LogEntry", back_populates="project", cascade="all, delete-orphan")
...
class Task(Base):
    ...
    log_entries: Mapped[list["LogEntry"]] = relationship("LogEntry", back_populates="task", cascade="all, delete-orphan")
    recurrence: Mapped[Optional["TaskRecurrence"]] = relationship("TaskRecurrence", back_populates="task", uselist=False, cascade="all, delete-orphan")
    migrations_from: Mapped[list["Migration"]] = relationship("Migration", foreign_keys="Migration.task_id", back_populates="task")
...
class Note(Base):
    ...
    log_entries: Mapped[list["LogEntry"]] = relationship("LogEntry", back_populates="note", cascade="all, delete-orphan")
...
class Event(Base):
    ...
    title: Mapped[str] = mapped_column(String(255), nullable=False)
    content: Mapped[Optional[str]] = mapped_column(Text)
    occurred_at: Mapped[datetime] = mapped_column(DateTime, nullable=False)  # When the event happened
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)

    # Relationships
    tags: Mapped[list["Tag"]] = relationship(secondary=event_tags, back_populates="events")
    log_entries: Mapped[list["LogEntry"]] = relationship("LogEntry", back_populates="event", cascade="all, delete-orphan")
...
class LogEntry(Base):
    ...
    task_id: Mapped[Optional[int]] = mapped_column(ForeignKey("task.id", ondelete="CASCADE"))
    note_id: Mapped[Optional[int]] = mapped_column(ForeignKey("note.id", ondelete="CASCADE"))
    event_id: Mapped[Optional[int]] = mapped_column(ForeignKey("event.id", ondelete="CASCADE"))
    entry_date: Mapped[datetime] = mapped_column(DateTime, nullable=False)  # The date this entry appears in the log
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
    ...
    migrations_to: Mapped[list["Migration"]] = relationship("Migration", foreign_keys="Migration.to_log_entry_id", back_populates="to_log_entry")
    migrations_from: Mapped[list["Migration"]] = relationship("Migration", foreign_keys="Migration.from_log_entry_id", back_populates="from_log_entry")
...
class TaskRecurrence(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    task_id: Mapped[int] = mapped_column(ForeignKey("task.id", ondelete="CASCADE"), unique=True, nullable=False)
    cron_expression: Mapped[str] = mapped_column(String(255), nullable=False)  # e.g., "0 9 * * MON" for 9am every Monday
...
class Migration(Base):
    ...
    from_log_entry_id: Mapped[int] = mapped_column(ForeignKey("log_entry.id", ondelete="CASCADE"), nullable=False)
    to_log_entry_id: Mapped[int] = mapped_column(ForeignKey("log_entry.id", ondelete="CASCADE"), nullable=False)
    ...
    from_log_entry: Mapped[LogEntry] = relationship("LogEntry", foreign_keys=[from_log_entry_id], back_populates="migrations_to")
    to_log_entry: Mapped[LogEntry] = relationship("LogEntry", foreign_keys=[to_log_entry_id], back_populates="migrations_from")
